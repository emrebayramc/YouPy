...
def _perform_args_on_youtube(youtube_item: YouTubeItem, args: argparse.Namespace) -> None:
    if args.list:
        display_streams(youtube_item)
    if args.build_playback_report:
        build_playback_report(youtube_item)
    if args.itag:
        download_by_itag(youtube_item=youtube_item, itag=args.itag, target=args.target)
    if hasattr(args, "caption_code"):
        download_caption(
            youtube_item=youtube_item, lang_code=args.caption_code, target=args.target
        )
    if args.resolution:
        download_by_resolution(
            youtube_item=youtube_item, resolution=args.resolution, target=args.target
        )
    if args.audio:
        download_audio(youtube_item=youtube_item, filetype=args.audio, target=args.target)
    if args.ffmpeg:
        ffmpeg_process(youtube_item=youtube_item, resolution=args.ffmpeg, target=args.target)
...
def _parse_args(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "The list option causes YouPy cli to return a list of streams "
...
def build_playback_report(youtube_item: YouTubeItem) -> None:
    ...
    fp = os.path.join(os.getcwd(), f"yt-video-{youtube_item.video_id}-{ts}.json.gz")

    js = youtube_item.js
    watch_html = youtube_item.watch_html
    vid_info = youtube_item.vid_info

    with gzip.open(fp, "wb") as fh:
        fh.write(
            json.dumps(
                {
                    "url": youtube_item.watch_url,
                    "js": js,
...
def ffmpeg_process(
    youtube_item: YouTubeItem, resolution: str, target: Optional[str] = None
) -> None:
    """
    Decides the correct video stream to download, then calls _ffmpeg_downloader.

    :param YouTubeItem youtube_item:
    ...
    :param str target:
        Target directory for download
    """
    youtube_item.register_on_progress_callback(on_progress)
    target = target or os.getcwd()

    if resolution == "best":
        highest_quality_stream = (
            youtube_item.streams.filter(progressive=False).order_by("resolution").last()
        )
        mp4_stream = (
            youtube_item.streams.filter(progressive=False, subtype="mp4")
            .order_by("resolution")
            .last()
        )
        ...
        video_stream = youtube_item.streams.filter(
    ...
    audio_stream = youtube_item.streams.get_audio_only(video_stream.subtype)
    if not audio_stream:
        audio_stream = youtube_item.streams.filter(only_audio=True).order_by("abr").last()
    if not audio_stream:
...
def download_by_itag(youtube_item: YouTubeItem, itag: int, target: Optional[str] = None) -> None:
    ...
    :param str target:
        Target directory for download
    """
    stream = youtube_item.streams.get_by_itag(itag)
    if stream is None:
...
def download_by_resolution(
    ...
    stream = youtube_item.streams.get_by_resolution(resolution)
    if stream is None:
        print(f"Could not find a stream with resolution: {resolution}")
        print("Try one of these:")
        display_streams(youtube_item)
...
def display_streams(youtube_item: YouTubeItem) -> None:
    ...
    """
    for stream in youtube_item.streams:
        print(stream)
...
def download_caption(
    youtube_item: YouTubeItem, lang_code: Optional[str], target: Optional[str] = None
) -> None:
    ...
    if lang_code is None:
        _print_available_captions(youtube_item.captions)
        return

    try:
        caption = youtube_item.captions[lang_code]
        downloaded_path = caption.download(title=youtube_item.title, output_path=target)
...
def download_audio(
    youtube_item: YouTubeItem, filetype: str, target: Optional[str] = None
) -> None:
    """
    ...
    """
    audio = (
        youtube_item.streams.filter(only_audio=True, subtype=filetype).order_by("abr").last()
    )

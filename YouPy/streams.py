...
class Stream:
    ...
    def __init__(self, stream: Dict, player_config_args: Dict, progress_state: ProgressState):
    ...
    def title(self) -> str:
        ...
        :returns:
            Youtube video title
        """
        return self._progress_state.title or "Unknown YouTube Video Title"
    ...
    def filesize_approx(self) -> int:
        ...
        """
        if self._progress_state.duration and self.bitrate:
            bits_in_byte = 8
            return int((self._progress_state.duration * self.bitrate) / bits_in_byte)
    ...
    def on_progress(self, chunk: bytes, file_handler: BinaryIO, bytes_remaining: int):
        ...
        if self._progress_state.on_progress:
            self._progress_state.on_progress(self, chunk, bytes_remaining)
    ...
    def on_complete(self, file_path: Optional[str]):
        ...
        on_complete = self._progress_state.on_complete
        if on_complete:
